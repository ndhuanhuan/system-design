...
This section provides a condensed version of the Google Docs design for quick review and interview preparation.

## 🎯 Core Problem

Design a real-time collaborative document editor where multiple users can edit the same document simultaneously and see each other's changes instantly.

## 📋 Key Requirements

**Functional:**
- Create documents
- Concurrent editing (≤100 users per doc)
...
- Low latency (<100ms)
- Scale to millions of users, billions of docs
- Durable & available

## 🏗️ Core Architecture Components

### 1. Document Metadata Service
- **Purpose**: Handle document CRUD operations
- **Tech**: REST API → Postgres (flexible querying, can partition later)
- **Stores**: Document metadata (title, owner, permissions, timestamps)

### 2. Document Service (Heart of the System)
- **Purpose**: Handle real-time collaborative editing
- **Tech**: WebSocket connections + Operational Transformation (OT)
- **Key Responsibilities**:
  - Maintain WebSocket connections to all editors
  - Apply & transform operations in order
  - Broadcast updates to all connected clients
  - Store cursor positions & user presence in-memory

### 3. Document Operations DB
- **Purpose**: Store append-only edit operations
- **Tech**: Cassandra (fast writes, partition by docId, order by timestamp)
- **Stores**: All edit operations (INSERT, DELETE) with timestamps

## 🔑 Critical Concepts

### Consistency Challenge: Why Simple Snapshots Fail

**Problem**: Two users editing simultaneously
- User A: Add ", world" to "Hello!" → "Hello, world!"
- User B: Delete "!" from "Hello!" → "Hello"

If we send full snapshots, last write wins → data loss!

### Solution 1: Operational Transformation (OT) ⭐ **[Google's Choice]**

**How it works:**
1. Send operations (INSERT, DELETE) not snapshots
2. Central server orders all operations
3. Transform operations based on what came before
4. Clients also transform incoming ops against local edits

**Example:**
- User B sends DELETE(6) to remove "!"
- User A's INSERT(5, ", world") arrives first
- Transform B's operation: DELETE(6) → DELETE(12)
- Result: "Hello, world" ✓

**Pros:**
- Low memory (only store operations, can compact)
- Fast
...
**How it works:**
1. Use real numbers for positions (can insert 4.5 between 4 and 5)
2. Keep tombstones for deleted text
3. Operations commute (can apply in any order)
...
**Cons:**
- Higher memory (never shrinks)
- Less elegant conflict handling
...
### Write Path
```
Editor → WebSocket → Document Service
    → Apply OT
    → Save to Operations DB (Cassandra)
...
### Read Path (Updates)
```
Editor A makes edit → Document Service
    → Broadcasts to Editor B, C, D...
    → Each client applies OT locally
```

### Cursor/Presence
```
Editor moves cursor → Document Service (in-memory)
...
### 1. Scale WebSocket Connections (Millions of Users)

**Problem**: Single server can't handle millions of connections

**Solution**: Consistent Hashing + ZooKeeper
1. Use consistent hash ring to distribute documents across servers
2. Hash document ID to determine server
3. All editors of same doc connect to same server
4. ZooKeeper maintains hash ring config

**Connection Flow:**
```
Client → Any server → Check hash ring 
...
- All editors of doc on same server (easy broadcasting)

**Challenges:**
- Connection redistribution during scaling events
- State management complexity
- Need robust monitoring & reconnection logic

### 2. Storage Optimization

**Problem**: Billions of docs × 50KB + millions of operations = massive storage

**Solution A**: Offline Compaction Service
- Separate service periodically compacts operations
- Use documentVersionId for atomicity
- Compact idle documents
- Race condition: avoid compacting active documents

**Solution B**: Online Compaction (Better!)
- Document Service compacts when last user disconnects
- Already has operations in memory
- Write compacted version under new documentVersionId
- Atomic flip in metadata DB

**Benefits:**
- Reduce storage significantly
- Faster document loads (fewer operations)
...
**Start Simple:**
1. Basic CRUD for documents
2. Add real-time editing (introduce consistency problem)
3. Explain OT vs CRDT tradeoffs
4. Choose OT for central server architecture

**Scale Gradually:**
1. Single server → Consistent hashing
2. Storage → Compaction strategies
3. Additional: read-only mode, versioning, offline support
...
- When to compact operations
- Memory vs storage tradeoffs

## 🔍 Complexity Analysis

**Per Document:**
- Concurrent editors: Up to 100
- Update latency: <100ms
- Memory: ~50KB + operations buffer

**System-wide:**
- Total users: Millions
- Total documents: Billions
- Active connections: Millions (distributed via consistent hashing)

## 🎯 Common Pitfalls to Avoid

1. ❌ Sending full document snapshots (inefficient + data loss)
2. ❌ Not handling operation ordering (consistency issues)
3. ❌ Forgetting to apply OT on client side (divergence)
4. ❌ Not addressing scaling WebSocket connections
5. ❌ Ignoring storage growth (operations accumulate)
6. ❌ Storing cursor position in database (should be ephemeral)

## 🏆 Excellence Signals

- Recognize consistency challenge immediately
- Compare OT vs CRDT knowledgeably
- Proactively address scaling WebSocket connections
- Consider storage optimization without prompting
