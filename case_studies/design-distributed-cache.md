...
![Simple Cache](design_distributed_cache/image-1.png)
...
![Simple LRU Cache](design_distributed_cache/image-3.png)
...
![Synchronous Replication](design_distributed_cache/image-4.png)
...
![Asynchronous Replication](design_distributed_cache/image-5.png)
...
![Peer-to-Peer Replication](design_distributed_cache/image-6.png)
...
This section provides a condensed version for quick interview preparation and review.

## 🎯 Core Problem

Design a distributed in-memory cache system that can store 1TB of data, handle 100k requests/second, with low latency (<10ms) and high availability.

## 📋 Key Requirements

**Functional:**
- Set, get, and delete key-value pairs (O(1) operations)
...
- Scalable to 1TB data and 100k requests/second
- NOT required: Durability, strong consistency, transactions
...
- Doubly linked list: Track access order for LRU
- Node structure: `{key, value, expiry, prev, next}`

**Why both?**
- Hash table alone: Fast lookup but can't track order
- List alone: Can track order but slow to find items
- Together: O(1) for all operations (get, set, delete, evict)
...
2. Check if expired → delete if yes, return null
...
2. If new key:
   - Create node with (key, value, expiry)
   - Add to hash table
   - Add to front of list
   - If at capacity: Remove LRU node from tail
```

### TTL & Cleanup
- Store expiry timestamp with each entry
- Lazy deletion: Check on get() operations
- Background janitor: Periodically scan and remove expired entries
...
**Storage needs:**
- 1TB total data / 24GB per node (32GB RAM - overhead) = ~43 nodes
- Add buffer: **50 nodes minimum**

**Throughput needs:**
- 100k requests/sec / 20k per node = 5 nodes
- Add buffer: ~8 nodes

**Result:** Storage is the bottleneck → provision 50 nodes

## 🚀 Scaling Strategies

### 1. Data Distribution (Sharding)

**Problem:** 1TB won't fit on single node

**Solution:** Consistent Hashing
- Arrange nodes in circular keyspace
- Hash keys to positions on circle
- Key stored on first node clockwise from position
- **Benefits:** 
  - Adding/removing nodes only affects adjacent keys
  - Much better than modulo (hash % N) which remaps everything

**Why not simple modulo?**
```
Modulo: hash(key) % 4 → If nodes change to 5, ALL keys remap
Consistent Hash: Only ~1/N keys remap when adding/removing nodes
```
...
- **Cons:** High latency, low availability (any replica down = system down)
- **Verdict:** Not suitable for cache (we want speed over consistency)

#### B) Asynchronous Replication ✅
- Write to primary, replicate to others asynchronously
- **Pros:** Fast writes, high availability, good scalability
- **Cons:** Temporary staleness, need failover mechanism
- **Verdict:** Good for most cache use cases (Redis default)

#### C) Peer-to-Peer Replication ✅
- All nodes equal, gossip protocol for propagation
- **Pros:** No single point of failure, maximum scalability
- **Cons:** Complex conflict resolution, eventual consistency
- **Verdict:** Great for massive scale (Cassandra-style)

### 3. Hot Key Handling

#### Hot Reads (High read traffic on specific keys)

**Problem:** Viral content hits one shard, overloads it

**Solutions:**

**a) Dedicated Hot Key Cache** ✅
- Separate tier with powerful hardware for hot keys
- Monitor and promote hot keys automatically
- Challenge: Consistency between tiers

**b) Read Replicas** ✅
- Replicate entire nodes
- Challenge: Storage overhead, replication lag

**c) Copies of Hot Keys** ⭐ **Best**
- Create multiple suffixed copies: `key#1`, `key#2`, `key#3`
- Distribute via consistent hashing
- Reads: Random suffix selection spreads load
- Writes: Update all copies (eventual consistency)
- Challenge: Only good for read-heavy keys

#### Hot Writes (High write traffic on specific keys)

**Problem:** Counter getting 10k writes/sec overwhelms one node

**Solutions:**

**a) Write Batching** ⭐
- Buffer writes for 50-100ms
- Apply as single operation
- 10k writes/sec → ~100 operations/sec
- Challenge: Delayed visibility, failure recovery

**b) Sharding Hot Key With Suffixes** ⭐
- Split into sub-keys: `counter:1` to `counter:10`
- Distribute writes across shards
- Read: Sum all shards
- Challenge: More complex reads, consistency across shards

## 🎨 API Design

```
POST /:key { "value": "...", "ttl": 3600 }
GET /:key → { "value": "..." }
...
### Connection Pooling
- Maintain persistent connections between client and servers
- Avoid handshake overhead for each request
- Critical for reducing tail latencies (p95, p99)
...
- Client-side routing (no central lookup service)
...
1. **Single node cache** (hash table + linked list for LRU)
2. **Add TTL support** (expiry timestamps + janitor process)
3. **Scale horizontally** (consistent hashing for distribution)
4. **Add replication** (async replication for availability)
5. **Handle hot keys** (read copies, write batching/sharding)

### Key Discussion Points
- **Why doubly linked list?** Explain O(1) for moving nodes
- **Consistent hashing details** Show understanding of remapping benefits
- **Replication trade-offs** Compare sync vs async vs P2P
- **Hot key strategies** Know multiple approaches and their trade-offs
- **When to use what** Match solution to specific requirements

### Common Trade-offs

**Consistency vs Availability:**
- Caches prioritize availability (eventual consistency)
- Temporary staleness acceptable for most use cases

**Memory vs Performance:**
- LRU requires extra memory for linked list
- But gives O(1) operations vs O(n) alternatives

**Simplicity vs Scalability:**
- Single node: Simple but limited
- Distributed: Complex but scales horizontally

## 🎯 Common Pitfalls

1. ❌ Forgetting TTL cleanup (memory leak)
2. ❌ Not explaining why you need BOTH hash table AND linked list
3. ❌ Using simple modulo instead of consistent hashing
4. ❌ Synchronous replication for a cache (wrong priority)
5. ❌ Not discussing hot key strategies
6. ❌ Ignoring connection pooling for performance
7. ❌ Not calculating how many nodes you actually need

## 🔍 Follow-up Questions to Expect

**Q: What happens when a node fails?**
A: Replicas take over, consistent hashing redistributes keys, clients retry on next node

**Q: How do you detect hot keys?**
A: Monitor access patterns, track request counts per key, set thresholds for promotion

**Q: Why not use Redis/Memcached?**
A: Great question! We're designing to understand principles. Production: use proven solutions.

**Q: How do you handle network partitions?**
A: Depends on replication model. Async: primaries continue serving, eventual consistency on heal. P2P: use quorum-based approaches.

**Q: What about cache stampede?**
A: When popular key expires, many requests hit database. Solutions: lock on cache miss, probabilistic early expiration, always return stale if exists.

**Q: How do you monitor cache performance?**
A: Track hit/miss ratio, latency (p50/p95/p99), eviction rate, memory usage, hot key metrics
...
Cache Cluster (50 nodes)
  ...
  - Async replication (3 replicas per key)
  - Each node: Hash table + Linked list
  - Hot key handling: Read copies, write batching
  ↓
Monitoring Service
  - Detect hot keys
...
├── TTL Janitor (cleanup expired keys)
└── Replication Manager (sync with replicas)
```

## 🏆 Excellence Signals

- Immediately recognize need for hash table + linked list combo
- Explain O(1) complexity for all operations clearly
- Proactively calculate capacity needs
- Discuss consistent hashing without prompting
- Compare multiple replication strategies with trade-offs
- Anticipate hot key question and have multiple solutions
- Mention connection pooling and batching for performance
- Understand eventual consistency is acceptable for caches
- Know when to use each hot key strategy (reads vs writes)

## 🎓 Related Concepts

- **Redis internals:** Similar design, adds persistence options
- **Memcached:** Simpler, pure cache, similar LRU approach
- **CDNs:** Geographic distribution of cached content
- **Browser caching:** Client-side caching strategies
- **Database query caches:** Application-level caching layer
